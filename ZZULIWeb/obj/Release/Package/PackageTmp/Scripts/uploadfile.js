$(function () {
    $('#uploadbtn').on("click", function () {
        ...
        if ($("input[id='Moves']").prop('files').length <= 0) {
            swal("请选择课程文件!");
            return;
        }
        if ($("input[id='Cou_Img']").prop('files').length <= 0) {
            swal("请选择课程图片文件!");
            return;
        }
        var couName = $('#Cou_Name').val().trim();
        if (couName == null || couName == "") {
            swal("请填写课程名称!");
            return;
        }
        var couDescribe = $('#Cou_Describe').val().trim();
        ...
        ajax.upload_big("/UpLoad/uploadbigfile", "#Moves", 1024 * 1024, "*", function () { alert("课程文件上传完毕..."); }, function (count, all) { console.log("当前传输进度：" + count + "/" + all); }, function (err) { console.log("Error:" + err.status); }, function () { console.log("timeout") });
        //alert("开始上传课程图片...");
        //uploadcourseimage();
    });
...
var initParam = {
    ...
    async: false,                                 //同|异步请求 （异步：true 同步：false）
...
var tempObj = {
    ...
    upload_big: function (url, fileSelector, cutSize, fileType, successEvent, progressEvent, errorEvent, timeoutEvent) {
        var file = $("input[id='Moves']").prop('files');
        var image = $("input[id='Cou_Img']").prop('files');
        var result = {};
        //以下为上传文件限制检查
        if (file.length === 1) {
            if (fileType != "*") {
         ...
         * */
        function cutFile_upload(fileArr, file) {
            var currentRequestIndex = 0;
            var retransNum_Part = 0;
            var retransNum = 0;
            while (currentRequestIndex < fileArr.length) {
                var formData = new FormData();
                formData.append("count", currentRequestIndex);
                formData.append("name", fileArr[currentRequestIndex].name);
                formData.append("file".name, fileArr[currentRequestIndex].file);
                formData.append("retransNum_Part", retransNum_Part);
                if (currentRequestIndex === fileArr.length - 1) {
                    formData.append("isLast", "true");
                    formData.append("Cou_Name", $('#Cou_Name').val());
                    formData.append("Cou_Describe", $('#Cou_Describe').val());
                    formData.append("Cou_Tag", $('#lb_nav1').val());
                    formData.append("Cou_Img", image[0]);
                    formData.append("retransNum", retransNum);
                }
                var ajaxParam = {
                    type: "post",
                    url: url,
                    ...
                    successEvent: function (data) {
                        ...
                        if (res.status == "1") {
                            currentRequestIndex++;
                            progressEvent(currentRequestIndex, fileArr.length);
                            retransNum_Part = 0;
                            retransNum = 0;

                            if (currentRequestIndex >= fileArr.length) {
                                successEvent();
                            }
                        }
                        else if (res.status == "2") {
                            console.log("error")
                            retransNum_Part = res.retransNum_Part;
                            if (retransNum_Part < 3) {
                                retransNum_Part++;
                            } else {
                                currentRequestIndex = 99999999;
                            }
                        } else if (res.status == "9") {
                            retransNum = res.retransNum;
                            if (retransNum < 3) {
                                retransNum++;
                            } else {
                                currentRequestIndex = 99999999;
                        ...
                        //progressEvent(currentRequestIndex, fileArr.length);   //上传进度事件，第一个参数：当前上传次数；第二个参数：总共文件数
                    },
                    errorEvent: errorEvent,
                    timeoutEvent: timeoutEvent
                };
